...
class Tensor {
  public:
    static Tensor createRandom(
        vax::core::RandomGenerator& generator, std::vector<int> shape, float min = 0.0f, float max = 1.0f
    );
    ...
    //     : _shape(shape) {
    //     _total_size = std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int>());
    ...
    // bool operator==(const Tensor& other) const { return approx_equal(other, 1e-5f); }
