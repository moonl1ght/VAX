...
std::optional<vax::vk::RenderPassDescriptor>
RenderPassDescriptorBuilder::buildMainSecondaryOffscreen(VkFormat imageFormat) const noexcept {
    VkAttachmentDescription colorAttachment{
        .format = imageFormat,
        .samples = VK_SAMPLE_COUNT_1_BIT,
        .loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR,
    ...
    VkRenderPass renderPass;
    if (!VK_CHECK(vkCreateRenderPass(_device.get().vkDevice, &renderPassInfo, nullptr, &renderPass))) {
        _logger.error("Failed to create secondary offscreen render pass!");
        return std::nullopt;
    }
